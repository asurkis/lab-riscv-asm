...
Дамп исходного кода на C &mdash; в `dump_c.txt`.

Дамп C через Godbolt &mdash; в `dump_c_godbolt.asm`
(машинные коды &mdash; в `dump_c_godbolt.txt`).
...
- GCC &mdash; семейство компиляторов, в т.ч. для RISC-V.
